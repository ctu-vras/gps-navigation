...
def gpx_to_waypoints(waypoints_density = 0.1, gpx_file="coords.gpx", waypoints_file="waypoints.csv", circular=False):     # waypoints_density == dist between neighbor waypoints. in meters
    if waypoints_density <= 0.:
	waypoints_density = 0.01
    gpx_f = open(gpx_file, 'r')
    gpx_object = gpxpy.parse(gpx_f)
    coords = np.array([[point.latitude,point.longitude] for point in gpx_object.waypoints])
    if circular:
...
if __name__ == "__main__":
    gpx_to_waypoints(waypoints_density = 0.1, gpx_file="test.gpx", waypoints_file="waypoints.csv", circular=True)
