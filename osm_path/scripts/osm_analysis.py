...
class PathAnalysis:
    ...
    def parse_rels(self):
        ...
        for rel in self.osm_rels_data.relations:
            if len(rel.members) <= MAX_REL_MEMBERS:   # A lot of members is very likely some relation we are not interested in.
                ...
                print("relation with {} members not used: {}".format(len(rel.members), rel.tags))
    ...
    def graph_search_path(self):
        ...
        while goal_points:              # Main cycle.
            ...
            while True:
                ...
                # TODO: Find a way to do this without the added for loop
                road_points_mask = []
                for i in range(len(road_polygons)):
                    road_points_mask.append(self.get_contain_mask(graph_points, road_polygons[i]))
                ...
                for i in range(ROAD_CROSSINGS_RANKS):
                    road_points.append((road_points_mask[i][edges[:,0]] + road_points_mask[i][edges[:,1]]) * (~footway_points_mask[edges[:,0]] + ~footway_points_mask[edges[:,1]]))
                ...
                #print("range {}, increase {}, density {}, points {}".format((2*increase_graph+INIT_WIDTH)*density,increase_graph, density, len(graph.get_vertices())))
                ...
                if shortest_path_vertices:
                    if (shortest_path_cost < MAX_COST_PER_METER * start_point.distance(goal_point)) or increase_graph > 0: #or graph_range >= MAX_RANGE:
                        ...
                        graph_counter += 1
                        print("Finished {}/{} sub-graphs.".format(graph_counter,num_sub_paths))
                        #print("solved in {}".format(time.time()-start_t))
                    ...
                    """ else:
                        if graph_range < MAX_RANGE:
                            ...
                            if detour_counter == 0:
                                print("Saving path and searching with new goal")
                    ...
                    if graph_range >= MAX_RANGE:
                        print("Sub-graph did not find solution in range {} m.".format(graph_range))
                        graph_counter += 1
                        break
                    else:
                        increase_graph += INCREASE
                ...
                """ if shortest_path_vertices and (shortest_path_cost < len(shortest_path_vertices) or increase_graph > 0):
                    ...
                    #print("COULD NOT FIND PATH IN {} M EFFECTIVE RANGE.".format((2*increase_graph+INIT_WIDTH)*density))
    ...
    def get_costs(self, p1, p2, roads, dist_cost, road_loss, no_footways, no_footway_loss):
        #return dist_cost
        #return np.reshape(np.sqrt(np.sum(np.square(p1-p2),axis=1)) + roads*road_loss + no_footway_loss * no_footways, (len(p1),1)) + dist_cost
    ...
    def get_road_crossing_cost(self):
        ...
        for i in range(len(ranked_segments)):
            for segment in ranked_segments[i]:
                ...
                ranked_segments_2[(ROAD_CROSSINGS_RANKS-1) if cost >= (ROAD_CROSSINGS_RANKS-1) else floor(cost)].append(segment.buffer(7/2))
