...
def rank_segments_curve(segments: list, junctions: list) -> None:
    ...
    for road in segments:
        ...
        road['curvature_level'][np.amax(np.amin(np.isin(road['coords'], junctions_coords), axis=2), axis=1)] = 5
...
def road_cost_for_curve(segments: list, exploration_limit: int = 100) -> list:
    ...
    for road in segments:
        for i in range(len(road)):
            ...
            # Append tuples, first is LineString object of road, second is the segment's curvature crossing rank
            ranked_segments.append((geom.LineString(road[i]['coords']), \
                                    (ROAD_CURVATURE_RANKS-1) if seg_value >= (ROAD_CURVATURE_RANKS-1) else floor(seg_value)))
